        ...
        <x-slot name="button">
            <div class="flex gap-x-2">
                <a href="{{ route('admin.atu.rank-seo.settings') }}"
                    class="bg-black text-white hover:bg-gray-800 px-3 py-2 rounded-md text-sm font-bold">
                    Global Settings
                </a>
                <a href="{{ route('admin.atu.rank-seo.media.index') }}"
                    class="bg-blue-500 text-white hover:bg-blue-600 px-3 py-2 rounded-md text-sm font-bold">
                    Media SEO
                </a>
            </div>
        </x-slot>
        ...
        <div class="overflow-hidden shadow-sm ring-1 ring-black/5 sm:rounded-lg mt-2">
            <table class="min-w-full divide-y divide-gray-300">
                <thead class="bg-gray-50">
                    <tr>
                        <th scope="col" class="py-3.5 pr-3 pl-4 text-left text-sm font-semibold text-gray-900 sm:pl-3">Slug Registry ID</th>
                        <th scope="col" class="py-3.5 pr-3 pl-4 text-left text-sm font-semibold text-gray-900 sm:pl-3">Type</th>
                    ...
                    @if ($this->seoEntries->isNotEmpty())
                        @foreach ($this->seoEntries as $entry)
                            <tr class="even:bg-gray-50">
                                <td class="py-4 pr-3 pl-4 text-sm font-medium whitespace-nowrap text-gray-900 sm:pl-3">{{ $entry->slug_registry_id ?? 'N/A' }}</td>
                                ...
                                <td class="px-3 py-4 text-sm whitespace-nowrap text-gray-500">{{ Str::limit($entry->title ?? 'N/A', 50) }}</td>
